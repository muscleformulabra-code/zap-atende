...
// Estado da conexão com o WhatsApp (consulta o conector). Usado pelo
// indicador "online/offline" no menu lateral.
export async function GET() {
  try {
    const r = await fetch(`${CONNECTOR_URL}/status`, { cache: 'no-store', signal: AbortSignal.timeout(4000) })
